...
test_that("examining a tree gives reasonable results", {
	...
	qrf = quantile_forest(seed=1000, X, Y, quantiles = c(0.1, 0.5, 0.9), mtry=p, min.node.size = 10, sample.fraction=0.632)
...
test_that("leaf samples are indexed correctly", {
	...
	qrf = quantile_forest(seed=1000, X, Y, sample.fraction=1.0, honesty=FALSE)
