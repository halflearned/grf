...
test_that("causal forests have reasonable split frequencies", {
    ...
    ccc = causal_forest(seed=1000, X, Y, W, mtry = p, imbalance.penalty=1.0, stabilize.splits=TRUE, min.node.size=2)
...
test_that("causal forests without stable splitting have reasonable split frequencies", {
    ...
    # Note that we increase imbalance.penalty to ensure the test reliably passes. Once
    # we add variance corrections, this should no longer be necessary.
    ccc = causal_forest(seed=1000, X, Y, W, mtry = p, imbalance.penalty=1.0, stabilize.splits=FALSE, min.node.size=2)
    split.freq = split_frequencies(ccc, 4)
    expect_true(split.freq[1,p] / sum(split.freq[1,]) > 2/3)
})
...
test_that("causal forests with a positive imbalance.penalty have reasonable tree depths", {
    ...
    forest = causal_forest(seed=1000, X, Y, W, imbalance.penalty=0.001)
...
test_that("causal forests with a very small imbalance.penalty behave similarly to unpenalized forests.", {
    ...
    forest = causal_forest(seed=1000, X, Y, W, imbalance.penalty=0.0)
    forest.large.penalty = causal_forest(seed=1000, X, Y, W, imbalance.penalty=100.0)
    forest.small.penalty = causal_forest(seed=1000, X, Y, W, imbalance.penalty=1e-6)
...
test_that("predictions are invariant to scaling of the sample weights.", {
   ...
   forest.1 = causal_forest(seed=1000, X, Y, W, sample.weights = sample.weights, seed=1)
   forest.2 = causal_forest(seed=1000, X, Y, W, sample.weights = 40*sample.weights, seed=1)
...
test_that("IPCC weighting in the training of a causal forest with missing data improves its complete-data MSE.", {
   ...
   num.trees = 400
   forest = causal_forest(seed=1000, X[cc,], Y[cc], W[cc], num.trees=num.trees)
   boosted.forest = causal_forest(seed=1000, X[cc,], Y[cc], W[cc], orthog.boosting = TRUE, num.trees = num.trees)
   weighted.forest = causal_forest(seed=1000, X[cc,], Y[cc], W[cc], sample.weights = sample.weights[cc], num.trees = num.trees)
   boosted.weighted.forest = causal_forest(seed=1000, X[cc,], Y[cc], W[cc], sample.weights = sample.weights[cc], orthog.boosting = TRUE, num.trees = num.trees)
...
test_that("Weighting is roughly equivalent to replication of samples", {
  ...
  regression.forest.rep = regression_forest(seed=1000, X.rep, W.rep, num.trees=num.trees)
  regression.forest.weight = regression_forest(seed=1000, X[rep(train, 2), ], c(W.a[train], W.b[train]),
    sample.weights=sample.weights, num.trees=num.trees)
  regression.forest.biased = regression_forest(seed=1000, X[rep(train, 2), ], c(W.a[train], W.b[train]),
    num.trees=num.trees)
  ...
  ## causal forest
  causal.forest.rep = causal_forest(seed=1000, X.rep, Y.rep, W.rep,
    W.hat = predict(regression.forest.rep)$predictions, num.trees=num.trees)
  causal.forest.weight = causal_forest(seed=1000, X[rep(train, 2),], c(Y.a[train],Y.b[train]), c(W.a[train],W.b[train]),
    W.hat = predict(regression.forest.weight)$predictions, sample.weights=sample.weights, num.trees=num.trees)
  causal.forest.biased = causal_forest(seed=1000, X[rep(train, 2),], c(Y.a[train],Y.b[train]), c(W.a[train],W.b[train]),
